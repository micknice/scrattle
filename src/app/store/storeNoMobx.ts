...
export class GameStore {
    ...
    private checkX(xYArray: TurnTileXY[]): boolean {
        const x1 = xYArray[0].xy[1]
        const check = xYArray.every((curr: TurnTileXY) => curr.xy[1] === x1)
        return check
    }
    ...
    private evaluateTurnScores(turnArray: TurnTile[]): TurnScoreObj {
        ...
        if (xYArray.length > 0 && this.checkX(xYArray)) {
        ...
        } else if (xYArray.length > 0 && this.checkY(xYArray)) {
            const xLineScores: XLineScoreObj[] = []
            const yLineScores: YLineScoreObj[] = []
